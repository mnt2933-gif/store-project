...
# رقم واتساب المحل - غيريه برقم المحل الحقيقي وقت التسليم
STORE_WHATSAPP = "218934741819"
STORE_NAME = "متجر تجريبي - Demo Store"
...
def admin_login():
    ...
    return render_template("admin_login.html", store_name=STORE_NAME, error=error)
...
def admin_change_password():
    ...
    return render_template(
        "admin_change_password.html", store_name=STORE_NAME, error=error, success=success
    )
...
def admin():
    ...
    return render_template("admin.html", products=products, store_name=STORE_NAME)
...
cat > templates/admin.html << 'ADMINEOF'
{% extends "layout.html" %}
{% block content %}

<div class="d-flex justify-content-between align-items-center mb-4">
    <h3>لوحة تحكم المحل</h3>
    <div>
        <a href="{{ url_for('admin_change_password') }}" class="btn btn-outline-secondary btn-sm">تغيير كلمة المرور</a>
        <a href="{{ url_for('admin_logout') }}" class="btn btn-outline-danger btn-sm">تسجيل خروج</a>
    </div>
</div>

<div class="alert alert-warning">
    ملاحظة: هذي نسخة ديمو بدون كلمة مرور. قبل التسليم الفعلي لازم نضيف حماية دخول.
</div>

<h5>إضافة جهاز جديد</h5>
<form method="post" action="{{ url_for('admin_add') }}" enctype="multipart/form-data" class="row g-2 mb-5">
    <div class="col-md-3"><input name="name" class="form-control" placeholder="اسم الجهاز" required></div>
    ...
    <div class="col-md-2"><input name="price" type="number" class="form-control" placeholder="السعر" required></div>
    <div class="col-md-3"><input name="specs" class="form-control" placeholder="المواصفات"></div>
    <div class="col-md-4"><input name="image_file" type="file" accept="image/*" class="form-control"></div>
...
<h5>المنتجات الحالية</h5>
<table class="table table-striped">
    <thead>
        <tr>
            <th>الاسم</th><th>الماركة</th><th>السعر</th><th>الحالة</th><th></th>
        </tr>
    </thead>
    <tbody>
        {% for p in products %}
        <tr>
            ...
            <td>{{ p['price'] }} د.ل</td>
            <td>
                <form method="post" action="{{ url_for('admin_toggle', product_id=p['id']) }}" class="d-inline">
                    <button class="btn btn-sm {% if p['available'] %}btn-success{% else %}btn-secondary{% endif %}">
                        {% if p['available'] %}متوفر{% else %}غير متوفر{% endif %}
                    </button>
                </form>
            </td>
            <td>
                <form method="post" action="{{ url_for('admin_delete', product_id=p['id']) }}" class="d-inline"
                      onsubmit="return confirm('متأكد تبي تحذف {{ p['name'] }}؟');">
...
cat > templates/admin_login.html << 'LOGINEOF'
{% extends "layout.html" %}
{% block content %}

<div class="row">
    <div class="col-md-4 offset-md-4">
        <h4 class="mb-4 text-center">دخول لوحة التحكم</h4>
        ...
        <form method="post">
            <input type="password" name="password" class="form-control mb-3" placeholder="كلمة المرور" required autofocus>
...
cat > templates/admin_change_password.html << 'PWEOF'
{% extends "layout.html" %}
{% block content %}
...
<div class="row">
    <div class="col-md-5 offset-md-3">
        ...
        <form method="post">
            <input type="password" name="current_password" class="form-control mb-2" placeholder="كلمة المرور الحالية" required>
            <input type="password" name="new_password" class="form-control mb-2" placeholder="كلمة المرور الجديدة" required>
            <input type="password" name="new_password_confirm" class="form-control mb-3" placeholder="تأكيد كلمة المرور الجديدة" required>
            <button class="btn btn-primary w-100">تحديث</button>
        </form>

        <a href="{{ url_for('admin') }}" class="btn btn-link mt-2">رجوع للوحة التحكم</a>
    </div>
</div>
...
cat > templates/index.html << 'INDEXEOF'
{% extends "layout.html" %}
{% block content %}

<form class="row g-2 mb-4" method="get">
    <div class="col-md-4">
        <input type="text" name="q" class="form-control" placeholder="ابحث عن جهاز..." value="{{ selected_q }}">
    </div>
    <div class="col-md-3">
        <select name="brand" class="form-select">
            <option value="">كل الماركات</option>
            {% for b in brands %}
                <option value="{{ b['brand'] }}" {% if b['brand'] == selected_brand %}selected{% endif %}>{{ b['brand'] }}</option>
            {% endfor %}
        </select>
    </div>
    <div class="col-md-3">
        <select name="category" class="form-select">
            ...
            <option value="phone" {% if selected_category == 'phone' %}selected{% endif %}>هواتف</option>
            <option value="laptop" {% if selected_category == 'laptop' %}selected{% endif %}>لابتوبات</option>
        </select>
    </div>
    <div class="col-md-2">
...
<div class="row">
    ...
    <div class="col-md-4 mb-4">
        <div class="card h-100 {% if not p['available'] %}opacity-50{% endif %}">
            {% if p['image_url'] %}
                <img src="{{ p['image_url'] }}" class="card-img-top" alt="{{ p['name'] }}" style="height: 200px; object-fit: contain; padding: 10px;">
            {% endif %}
            <div class="card-body">
                <h5 class="card-title">{{ p['name'] }}</h5>
                <h6 class="card-subtitle mb-2 text-muted">{{ p['brand'] }}</h6>
                <p class="card-text">{{ p['specs'] }}</p>
                <p class="fw-bold">{{ p['price'] }} د.ل</p>
                {% if p['available'] %}
                    <span class="badge bg-success mb-2">متوفر</span>
                {% else %}
                    <span class="badge bg-secondary mb-2">غير متوفر</span>
                {% endif %}
                <a href="{{ url_for('product_detail', product_id=p['id']) }}" class="btn btn-outline-primary w-100">التفاصيل</a>
            </div>
        </div>
    </div>
    {% else %}
    <p class="text-center text-muted">ما فيه نتائج مطابقة.</p>
    {% endfor %}
</div>
...
cat > templates/product.html << 'PRODEOF'
{% extends "layout.html" %}
{% block content %}
...
<div class="row">
    <div class="col-md-8 offset-md-2">
        {% if product['image_url'] %}
            <img src="{{ product['image_url'] }}" class="img-fluid mb-3" style="max-height: 300px; object-fit: contain;" alt="{{ product['name'] }}">
        {% endif %}
        <h2>{{ product['name'] }}</h2>
        <h5 class="text-muted">{{ product['brand'] }}</h5>
        <p class="mt-3">{{ product['specs'] }}</p>
        <p class="fs-4 fw-bold">{{ product['price'] }} د.ل</p>

        {% if product['available'] %}
            <span class="badge bg-success mb-3">متوفر الآن</span>
        {% else %}
            <span class="badge bg-secondary mb-3">غير متوفر حالياً</span>
        {% endif %}
...
echo 'تم تحديث كل الملفات بنجاح'
